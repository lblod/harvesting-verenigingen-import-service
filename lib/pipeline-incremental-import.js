...
import { storeToNTriples, storeAsArray } from './utils';
...
export async function run(task) {
  ...
  try {
    ...
    for(const verenigingSubject of verenigingenSubjects) {
...
async function updateMutatiedienstStateInfo(store) {
  let sequence = store
        .getObjects(undefined,
                    namedNode("http://data.lblod.info/vocabularies/FeitelijkeVerenigingen/lastSequenceMutatiedienst")
                   )[0];

  sequence = rst.termToString(sequence);

  const queryStr = `
    DELETE {
      GRAPH ?g {
        ?s a <http://data.lblod.info/vocabularies/FeitelijkeVerenigingen/MutatiedienstStateInfo>;
          <http://purl.org/dc/terms/modified> ?modified;
          <http://data.lblod.info/vocabularies/FeitelijkeVerenigingen/lastSequenceMutatiedienst> ?lastSequence.
      }
    }
    INSERT {
      GRAPH ?g {
        ?s a <http://data.lblod.info/vocabularies/FeitelijkeVerenigingen/MutatiedienstStateInfo>;
          <http://purl.org/dc/terms/modified> ${sparqlEscapeDateTime((new Date()).toISOString())};
          <http://data.lblod.info/vocabularies/FeitelijkeVerenigingen/lastSequenceMutatiedienst> ${sequence}.
      }
    }
    WHERE {
      GRAPH ?g {
